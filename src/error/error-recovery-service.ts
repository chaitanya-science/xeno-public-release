...
export class ErrorRecoveryServiceImpl implements ErrorRecoveryService {
  ...
  private createDefaultHardwareMonitor(): HardwareMonitor {
    return {
  ...
  async handleHardwareError(error: HardwareError): Promise<RecoveryAction> {
  ...
  async handleAPIError(error: APIError): Promise<RecoveryAction> {
  ...
  async exitOfflineMode(): Promise<void> {
  ...
  async getCircuitBreakerStatus?(serviceKey: string): Promise<'OPEN' | 'CLOSED' | 'HALF_OPEN'> {
    // Simple stub – always closed for tests
    return 'CLOSED';
  }

  private async handleConnectionLoss(): Promise<RecoveryAction> {
  ...
  private async handleAPITimeout(error: NetworkError): Promise<RecoveryAction> {
    if (error.retryable) {
      return {
        type: 'RETRY',
        delay: 5000,
  ...
  private async handleDNSFailure(): Promise<RecoveryAction> {
    return {
      type: 'OFFLINE_MODE',
  ...
  private async handleRateLimit(error: NetworkError): Promise<RecoveryAction> {
  ...
  private async handleMicrophoneFailure(error: HardwareError): Promise<RecoveryAction> {
  ...
  private async handleSpeakerFailure(error: HardwareError): Promise<RecoveryAction> {
  ...
  private async handleMemoryExhaustion(error: HardwareError): Promise<RecoveryAction> {
  ...
  private async handleCPUOverload(error: HardwareError): Promise<RecoveryAction> {
    return {
      type: 'FALLBACK',
  ...
  private async handleAuthenticationError(error: APIError): Promise<RecoveryAction> {
    return {
      type: 'OFFLINE_MODE',
  ...
  private async handleQuotaExceeded(error: APIError): Promise<RecoveryAction> {
    return {
      type: 'FALLBACK',
      fallbackService: 'cached_responses',
  ...
  private async handleServiceUnavailable(error: APIError): Promise<RecoveryAction> {
    if (error.retryable) {
      return {
        type: 'RETRY',
        delay: 30000,
  ...
  private async handleInvalidRequest(error: APIError): Promise<RecoveryAction> {
    return {
      type: 'FALLBACK',
  ...
  private async handleGenericNetworkError(error: NetworkError): Promise<RecoveryAction> {
    return {
      type: 'RETRY',
      delay: 5000,
  ...
  private async handleGenericHardwareError(error: HardwareError): Promise<RecoveryAction> {
    return {
      type: 'USER_NOTIFICATION',
  ...
  private async handleGenericAPIError(error: APIError): Promise<RecoveryAction> {
    return {
      type: 'FALLBACK',
      fallbackService: 'basic_responses',
  ...
  private async checkNetworkRecovery(): Promise<{ recovered: boolean }> {
