...
import { VoiceAssistantManager, VoiceSessionState } from '../voice/voice-assistant-manager';
...
class MockSpeechToTextService {
  ...
  async adaptToSpeaker(userId: string) {}
  dispose() {}
}
...
class MockTextToSpeechService {
  ...
  async synthesizeSpeechWithEmotion(text: string, emotion: any) {
    return {} as any;
  }
  
  async playAudio(audioBuffer: any) {}
  async speakWithEmotion(text: string, emotion: any) {}
  async handleTechnicalDelay(reason: string) {}
...
class MockConversationManager {
  ...
  async updateEmotionalContext(sessionId: string, emotion: any) {}
}
...
async function runVoiceAssistantDemo() {
  ...
  const wakeWordDetector = new MockWakeWordDetector() as any;
  const speechToTextService = new MockSpeechToTextService() as any;
  const textToSpeechService = new MockTextToSpeechService() as any;
  const conversationManager = new MockConversationManager() as any;
  ...
  voiceAssistant.on('state_changed', (event: any) => {
    console.log(`🔄 State changed: ${event.from} → ${event.to}`);
  });

  voiceAssistant.on('session_started', (event: any) => {
  ...
  voiceAssistant.on('speech_transcribed', (event: any) => {
    console.log(`📝 User said: "${event.text}" (confidence: ${event.confidence})`);
  });

  voiceAssistant.on('response_generated', (event: any) => {
