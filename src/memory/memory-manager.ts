...
import { Memory, MemoryType, PrivacySettings } from '../types';
import { MemoryManager } from './interfaces';
import { SQLiteStorageService } from './storage-service';

export class WellnessMemoryManager implements MemoryManager {
  private storageService: SQLiteStorageService;
  protected get MAX_MEMORIES_PER_USER() { return 1000; }
  private readonly IMPORTANCE_DECAY_FACTOR = 0.95;
  private readonly DAYS_TO_MILLISECONDS = 24 * 60 * 60 * 1000;
  ...
  async storeMemory(userId: string, memory: Omit<Memory, 'memory_id' | 'created_date' | 'importance_score'> & { importance_score?: number }): Promise<string> {
    // Auto-fill missing last_referenced if not provided
    const now = new Date();
    const memoryWithRef: any = { ...memory };
    if (!memoryWithRef.last_referenced) memoryWithRef.last_referenced = now;
  ...
  async retrieveMemories(userId: string, queryOrLimit?: string | number, maybeLimit?: number): Promise<Memory[]> {
  ...
  private getTypeImportanceMultiplier(memoryType: MemoryType): number {
    switch (memoryType) {
