...
import { ContinuousAudioManager } from '../audio/continuous-audio-manager';
...
export class ContinuousVoiceAssistant extends EventEmitter {
  private state: VoiceState = VoiceState.IDLE;
  private audioManager: ContinuousAudioManager;
  private currentSessionId: string | null = null;
  private config: ContinuousVoiceConfig;
  ...
  constructor(
    ...
    config: Partial<ContinuousVoiceConfig> = {},
    audioManagerConfig: Partial<import('../audio/continuous-audio-manager').ContinuousAudioConfig> = {},
    logger: any,
  ) {
    super();
    this.logger = logger;
    ...
    this.audioManager = new ContinuousAudioManager(
      speechToTextService,
      textToSpeechService,
      audioManagerConfig
    );
   ...
   */
  private async handleUserInput(text: string, confidence: number): Promise<void> {
    try {
      ...
      const responseText = await this.conversationManager.processMessage(
   ...
   * Handle low confidence transcription
   */
  private async handleLowConfidence(text: string, confidence: number): Promise<void> {
    this.retryCount++;
   ...
   */
  private isEndCommand(text: string): boolean {
    const endCommands = [
   ...
   */
  private async handleEndCommand(text: string): Promise<void> {
   ...
   * Refresh session timer
   */
  private refreshSessionTimer(): void {
    this.startSessionTimer();
  }
  ...
  private async processUserResponse(text: string): Promise<void> {
    this.setState(VoiceState.RESPONDING);
