...
import { ConversationSession, Speaker } from '../types';
import { ConversationFlowController } from './conversation-flow';
import { AIService } from './ai-service';
import { ComprehensiveCrisisResourceProvider, CrisisType } from '../crisis';
 ...
 */
export class ResponseRouter implements IResponseRouter {
  private conversationFlow: ConversationFlowController;
  private aiService?: AIService;
  private resourceProvider: ComprehensiveCrisisResourceProvider;

  constructor(conversationFlow: ConversationFlowController, aiService?: AIService) {
    this.conversationFlow = conversationFlow;
    this.aiService = aiService;
    this.resourceProvider = new ComprehensiveCrisisResourceProvider();
  }
  ...
  async routeResponse(
    ...
    switch (responseType) {
      ...
      case ResponseType.GENERAL_CONVERSATION:
      default:
   ...
   */
  private async generateCrisisResponse(message: string, context: ConversationSession): Promise<string> {
    try {
      ...
      if (crisisAnalysis.crisisType === CrisisType.SELF_HARM) {
      ...
      } else if (crisisAnalysis.crisisType === CrisisType.MEDICAL_EMERGENCY) {
      ...
      } else if (crisisAnalysis.crisisType === CrisisType.SEVERE_DISTRESS) {
   ...
   */
  private generateBasicCrisisResponse(message: string): string {
   ...
   */
  private generateEmpatheticResponse(message: string, context: ConversationSession): string {
    ...
    if (emotionalState.valence < -0.5) {
      ...
      supportiveResponses.push(
        "I can hear the pain in what you're sharing. That sounds really difficult to carry.",
        "It takes courage to talk about these feelings. I'm here with you through this.",
        "What you're going through sounds overwhelming. You're not alone in feeling this way."
      );
    } else if (emotionalState.arousal > 0.6) {
      // High arousal - provide calming presence
      supportiveResponses.push(
        "I can sense there's a lot of intensity in what you're experiencing. Let's take this one step at a time.",
        "It sounds like there's a lot happening for you right now. I'm here to listen at whatever pace feels right.",
        "That sounds like a lot to process. Would it help to talk through what's feeling most pressing right now?"
      );
    } else {
      // General empathetic responses
      supportiveResponses.push(
        "Thank you for trusting me with this. I can hear how important this is to you.",
        "I'm listening, and I want you to know that your feelings are valid.",
        "It sounds like you're dealing with something meaningful. I'm here to support you through it."
      );
    }
   ...
   */
  private generateFollowUpQuestion(message: string, context: ConversationSession): string {
    const lowerMessage = message.toLowerCase();
   ...
   */
  private generateClarificationRequest(message: string, context: ConversationSession): string {
   ...
   */
  private generateGeneralResponse(message: string, context: ConversationSession): string {
    const lowerMessage = message.toLowerCase();
   ...
   */
  private containsSelfHarmKeywords(message: string): boolean {
   ...
   */
  private containsMedicalEmergencyKeywords(message: string): boolean {
   ...
   */
  private containsPositiveKeywords(message: string): boolean {
   ...
   */
  async analyzeEmotionalTone(message: string) {
    if (this.aiService) {
      try {
   ...
   */
  private basicEmotionalAnalysis(message: string) {
    const lowerMessage = message.toLowerCase();
