...
import { createOpenAIService, validateOpenAIApiKey } from '../ai-service';
...
async function demonstrateAIIntegration() {
  ...
  const responseRouter = new ResponseRouter(conversationFlow, aiService);
  ...
  try {
    ...
    const positiveResponse = await conversationManager.processMessage(
      session.session_id,
    ...
    const emotionalState = await responseRouter.analyzeEmotionalTone(
      'I am feeling overwhelmed and anxious about everything'
    );
  ...
  try {
    const fallbackService = createOpenAIService('sk-invalid-key-for-testing');
    const fallbackRouter = new ResponseRouter(conversationFlow, fallbackService);
    
    const fallbackResponse = await fallbackRouter.routeResponse(
      'I need support',
      {
...
async function testResponsePerformance() {
  ...
  if (!validateOpenAIApiKey(config.ai.openai.apiKey)) {
    console.log('⚠️  Skipping performance test - no valid API key');
    return;
  }

  const aiService = createOpenAIService(config.ai.openai.apiKey, {
...
export { demonstrateAIIntegration, testResponsePerformance };