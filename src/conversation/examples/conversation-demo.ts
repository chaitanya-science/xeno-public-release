...
async function demonstrateConversationSystem() {
  ...
  try {
    ...
    const responseRouter = new ResponseRouter(flowController);
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const workResponse = await conversationManager.processMessage(
        session.session_id,
        `I hate my job so much, it's terrible ${i + 1}`
      );
    ...
    const anxiousResponse = await conversationManager.processMessage(
      session.session_id,
...
export { demonstrateConversationSystem };