...
export class OpenAIService implements AIService {
  ...
  private therapeuticConfig: TherapeuticPromptConfig;
  ...
  async generateResponse(
    ...
    const cachedResponse = performanceManager.getCachedResponse(cacheKey, JSON.stringify(context.conversation_history.slice(-3)));
    ...
    try {
      ...
      performanceManager.cacheResponse(cacheKey, JSON.stringify(context.conversation_history.slice(-3)), validatedResponse);
...
Only respond with the JSON object, no other text.`;
   ...
   */
  private getDefaultSystemPrompts(): Record<ResponseType, string> {
...
- Acknowledge your limitations as an AI companion named Xeno`;
    ...
    return {
      [ResponseType.EMPATHETIC_SUPPORT]: `${baseTherapeuticPrinciples}
...
- Help them feel less alone in their experience`,
      ...
      [ResponseType.FOLLOW_UP_QUESTION]: `${baseTherapeuticPrinciples}
...
- Help them explore connections between thoughts, feelings, and experiences`,
      ...
      [ResponseType.TOPIC_GUIDANCE]: `${baseTherapeuticPrinciples}
...
- Suggest topics that might bring comfort, hope, or positive reflection`,
      ...
      [ResponseType.CRISIS_RESPONSE]: `${baseTherapeuticPrinciples}
...
- Focus on immediate safety and connection to professional resources`,
      ...
      [ResponseType.CLARIFICATION_REQUEST]: `${baseTherapeuticPrinciples}
...
- Make them feel heard even when asking for clarification`,
      ...
      [ResponseType.GENERAL_CONVERSATION]: `${baseTherapeuticPrinciples}
...
- Be a caring companion who is fully present in the moment`
   ...
   */
  private buildTherapeuticSystemPrompt(responseType: ResponseType, emotionalState: EmotionalState): string {
   ...
   */
  private buildEmotionalAdaptation(emotionalState: EmotionalState): string {
   ...
   */
  private buildEmpathyAdjustment(): string {
   ...
   */
  private buildAdaptedPrompt(message: string, context: ConversationSession, responseType: ResponseType): string {
...
Please respond as a compassionate AI wellness companion, following your therapeutic guidelines and adapting to the user's emotional state.`;
   ...
   */
  private buildConversationContext(context: ConversationSession): string {
    ...
    const contextMessages = recentMessages.map(msg => 
      `${msg.speaker === 'USER' ? 'User' : 'You'}: ${msg.content}`
    ).join('\n');
  ...
  private buildEmotionalContext(emotionalState: EmotionalState): string {
    ...
    return `User's current emotional state: ${valenceDesc} mood with ${arousalDesc}. ${
      emotionalState.dominant_emotion ? `Primary emotion: ${emotionalState.dominant_emotion}.` : ''
    }`;
  }
   ...
   */
  private buildConversationMessages(context: ConversationSession) {
   ...
   */
  private getAdaptedTemperature(responseType: ResponseType, emotionalState: EmotionalState): number {
   ...
   */
  private validateAndCleanResponse(response: string, responseType: ResponseType): string {
   ...
   */
  private getFallbackResponse(responseType: ResponseType): string {
    const fallbacks = {
      [ResponseType.EMPATHETIC_SUPPORT]: "I can hear that this is important to you. I'm here to listen and support you through this.",
      [ResponseType.FOLLOW_UP_QUESTION]: "Can you tell me more about what that's like for you?",
      [ResponseType.TOPIC_GUIDANCE]: "Would you like to explore something different that's been on your mind?",
      [ResponseType.CRISIS_RESPONSE]: "I'm concerned about you. Please consider reaching out to a crisis helpline at 988 for immediate support.",
      [ResponseType.CLARIFICATION_REQUEST]: "I want to make sure I understand you correctly. Could you tell me a bit more?",
      [ResponseType.GENERAL_CONVERSATION]: "I'm listening. Please, tell me more."
    };
   ...
   */
  private validateEmotionalState(state: EmotionalState): EmotionalState {
    return {
   ...
   */
  private enhancedSentimentAnalysis(message: string): EmotionalState {
   ...
   */
  private basicSentimentAnalysis(message: string): EmotionalState {
...
export function createOpenAIService(
  ...
  const config: ConversationAIConfig = {
    apiKey: apiKey.trim(),
