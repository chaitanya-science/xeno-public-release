...
export class GoogleSpeechToTextService implements SpeechToTextService {
  ...
  private readonly RECOGNITION_TIMEOUT_MS = 20000; // 20 second timeout to accommodate full capture
  private readonly MIN_CONFIDENCE_THRESHOLD = 0.2; // Minimum confidence for acceptance (not strictly enforced)
  private readonly MAX_ALTERNATIVES = 3; // Number of alternative transcriptions to request
  ...
  constructor(
    ...
    this.audioProcessor = audioProcessor || new BasicAudioProcessor();
  }
   ...
   */
  async transcribe(audioBuffer: Buffer): Promise<SpeechRecognitionResult> {
  ...
  const processedAudio = await this.audioProcessor.processAudio(audioBuffer);
      ...
      if (error instanceof Error) {
        if (error.message.includes('timeout') || error.message.includes('DEADLINE_EXCEEDED')) {
          ...
          return this.createErrorResult(
            'I\'m having trouble with my connection. Please try again in a moment.',
            0,
            processingTime
          );
        } else if (error.message.includes('INVALID_ARGUMENT')) {
          return this.createErrorResult(
            'I\'m having trouble processing the audio. Could you try speaking again?',
            0,
            processingTime
          );
        }
      ...
      return this.createErrorResult(
        'I\'m sorry, I didn\'t understand that. Could you please speak a bit more clearly?',
        0,
        processingTime
      );
    }
   ...
   */
  async adaptToSpeaker(userId: string): Promise<void> {
