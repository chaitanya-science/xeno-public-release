...
import { TextToSpeechService, TTSConfig, EmotionalVoiceMapping } from './tts-interfaces';
...
export class TTSFactory {
  static createTTSService(config: SystemConfig): TextToSpeechService {
    ...
    return new GoogleTextToSpeechService(ttsConfig);
  }

  private static buildTTSConfig(config: SystemConfig): TTSConfig {
    return {
  ...
  private static getDefaultEmotionalMappings(): EmotionalVoiceMapping[] {
    return [
      // Happy and energetic
      {
        valence: 0.8,
