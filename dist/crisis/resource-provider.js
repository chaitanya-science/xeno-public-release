...
class ComprehensiveCrisisResourceProvider {
     ...
     */
    async getCrisisResources(crisisType) {
        const resources = this.crisisResources.get(crisisType) || [];
        // Always include general mental health resources for severe distress
        if (crisisType === interfaces_1.CrisisType.SEVERE_DISTRESS) {
            const generalResources = this.crisisResources.get(interfaces_1.CrisisType.NONE) || [];
     ...
     */
    formatResourceMessage(resources) {
        ...
        if (resourcesByType.has(interfaces_1.CrisisType.SELF_HARM)) {
            message += "🆘 **Immediate Crisis Support:**\n";
            resourcesByType.get(interfaces_1.CrisisType.SELF_HARM).forEach(resource => {
                message += this.formatSingleResource(resource);
            });
            message += "\n";
        }
        // Format medical emergency resources
        if (resourcesByType.has(interfaces_1.CrisisType.MEDICAL_EMERGENCY)) {
            message += "🚨 **Emergency Medical Help:**\n";
            resourcesByType.get(interfaces_1.CrisisType.MEDICAL_EMERGENCY).forEach(resource => {
                message += this.formatSingleResource(resource);
            });
            message += "\n";
        }
        // Format mental health resources
        if (resourcesByType.has(interfaces_1.CrisisType.SEVERE_DISTRESS)) {
            message += "💙 **Mental Health Support:**\n";
            resourcesByType.get(interfaces_1.CrisisType.SEVERE_DISTRESS).forEach(resource => {
                message += this.formatSingleResource(resource);
            });
            message += "\n";
        }
        // Add general support resources
        if (resourcesByType.has(interfaces_1.CrisisType.NONE)) {
            message += "🤝 **Additional Support:**\n";
            resourcesByType.get(interfaces_1.CrisisType.NONE).forEach(resource => {
                message += this.formatSingleResource(resource);
            });
        }
     ...
     */
    async getResourcesByAvailability(availability) {
        const allResources = [];
    ...
    async searchResources(keyword) {
        ...
        for (const resources of this.crisisResources.values()) {
            allResources.push(...resources.filter(r => r.name.toLowerCase().includes(searchTerm) ||
    ...
    initializeCrisisResources() {
        ...
        this.crisisResources.set(interfaces_1.CrisisType.SELF_HARM, [
            {
                ...
                availability: "24/7",
                type: interfaces_1.CrisisType.SELF_HARM
            },
            {
        ...
        this.crisisResources.set(interfaces_1.CrisisType.MEDICAL_EMERGENCY, [
            {
                name: "Emergency Services",
                phone: "911",
                description: "Immediate emergency medical response",
                availability: "24/7",
                type: interfaces_1.CrisisType.MEDICAL_EMERGENCY
            },
            {
        ...
        this.crisisResources.set(interfaces_1.CrisisType.SEVERE_DISTRESS, [
            {
                ...
                type: interfaces_1.CrisisType.SEVERE_DISTRESS
            },
            {
        ...
        this.crisisResources.set(interfaces_1.CrisisType.NONE, [
            {
                name: "211 Helpline",
                phone: "211",
                ...
                availability: "24/7",
                type: interfaces_1.CrisisType.NONE
            },
            {
...
exports.ComprehensiveCrisisResourceProvider = ComprehensiveCrisisResourceProvider;
